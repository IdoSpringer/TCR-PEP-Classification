...
def load_pair_data(pairfile):
    with open(pairfile, 'r') as file:
        ...
        for line in file:
            ...
            # stop signal
            tcrs.append(tcr + 'X')
            # assume only one peptide for tcr?
            try:
                pep_of_tcr[tcr].append(pep)
            except KeyError:
                pep_of_tcr[tcr] = [pep]
    return tcrs, pep_of_tcr
...
def train_epoch(batches, batch_size, model, loss_function, optimizer, device):
    ...
    for batch in batches:
        padded_tcrs = batch
        # concats = batch.view((batch_size, -1))
        # Move to GPU
...
def evaluate(batches, batch_size, model, ix_to_amino, device):
    ...
    with open('full_ae_accuracy.txt', 'a+') as file:
...
def main():
    # pairfile = 'pair_sampling/pairs_data/weizmann_pairs.txt'
    shirit_data = 'BM_data_CDR3s'
    ...
    batch_size = 50
    # tcrs, pep_of_tcr = load_data(pairfile)
    tcrs = load_all_data(shirit_data)
    print(len(tcrs))
    ...
    device = 'cuda:0'
    model = train_model(train_batches, batch_size, max_len, encoding_dim=30, epochs=300, device=device)
